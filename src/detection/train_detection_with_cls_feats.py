...
def get_cls_model(device, cls_model_folder):
    ...
    timm_fpn_backbone.change_layer(model, torch.nn.BatchNorm2d, torchvision.ops.misc.FrozenBatchNorm2d, params_map={'num_features':'num_features', 'eps':'eps'})
    print('classification model load_state_dict:', model.load_state_dict(ckpt['model'], strict=False))
    return model.to(device), ckpt['args_dict']['model_input_size'], ckpt['norm_mean'], ckpt['norm_std'], ckpt['args_dict']['model_name']
...
cls_model, cls_size, cls_norm_mean, cls_norm_std, cls_model_name = get_cls_model(device, args_dict['cls_model_folder'])
