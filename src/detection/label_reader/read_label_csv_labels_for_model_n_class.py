...
def revise_bbox(bbox):
    ...
    """
    x1, y1, x2, y2 = bbox
    if abs(x1-x2) < 1 or abs(y1-y2) < 1: return None, False
...
def read_label(label_file, split=None, force_binary=False, only_class=None):
    ...
    for idx, accno, label_str in tqdm(data[['ACCNO', 'labels_for_model']].itertuples()):
...
def get_label(idx, accno, label_str):
        ...
        valid_labels = [(bbox, cls) for bbox, cls, valid in (convert_bbox_str_to_bbox(bbox_str) for bbox_str in bbox_strs) if valid]
