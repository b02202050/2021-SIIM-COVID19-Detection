...
source_paths = []
target_paths = []
for source_folder, folders, files in os.walk(source_root_folder, followlinks=True):
    for file in files:
...
with Pool(16) as pool:
    out_stat = [(idx, reason) for idx, reason in tqdm(pool.imap(transfer, enumerate(zip(source_paths, target_paths)))) if reason != '']
if len(out_stat) > 0:
